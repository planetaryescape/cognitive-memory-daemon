...
- Unix socket is mode `0700`. The identity-scoped runtime parent directory is mode `0700`. The OS denies non-owner access.
- SQLite file `data.db` is mode `0600`. Created with `umask 077` set on daemon startup before any file is touched.
- PID file is mode `0600`. Identity-scoped daemon log files are mode `0600`.
...
- Keys are redacted in tracing fields and error messages. A key never appears in the daemon log file reported by `cm status`.
...
- `crates/daemon/tests/workspace_boundaries.rs` fails if `TcpListener` appears under `crates/daemon/src`.

### T5 — HTTP bridge

- `cm-http` binds `127.0.0.1` only. Loopback only.
- Every request requires `Authorization: Bearer <token>`. Tokens are minted by `Diagnostics::MintBridgeToken` and stored in `kv`. Token format: `cmb_<64 hex chars>` from two UUIDv4 values. Token entropy is ≥ 192 bits.
...
- The bridge validates daemon-minted tokens against the daemon on each request, supports an in-memory hashed bootstrap token map for local scripts/tests, rejects disallowed Host headers, and applies an explicit CORS allowlist.
- Bridge logs do not include `Authorization` headers or request bodies.
...
- Token randomness: UUIDv4 randomness from the `uuid` crate, backed by the OS CSPRNG.
